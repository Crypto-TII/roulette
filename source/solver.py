...
def poly_multiplier_to_matrix(poly, row_index=None):
	...
	if row_index is None:
		...
		r = np.concatenate((np.flip(p[:row_index+1]), -np.flip(p[row_index+1:])))
	return r
...
def manipulate_ciphertext(pke, c, index):
	...
	error = KyberPKE.decompress(v_mod, pke.DV) - KyberPKE.decompress(v[index], pke.DV)
	error %= KyberPKE.Q
	v[index] = v_mod
...
def generate_inequalities(
		...
		verbose=True):
	if verbose:
		print("Generating {:d} inequalities for {:s}...".format(nb_of_inequalities, kem.version()))
	...
	if return_manipulation:
		...
		ciphertexts = np.zeros((nb_of_inequalities, (K*kem.pke.DU + kem.pke.DV)*N//8), dtype=np.uint8)
		manipulated_ciphertexts = np.zeros((nb_of_inequalities, (K*kem.pke.DU + kem.pke.DV)*N//8), dtype=np.uint8)
		shared_secrets = np.zeros((nb_of_inequalities, KyberKEM.SHARED_SECRET_BYTES), dtype=np.uint8)
		manipulated_shared_secrets = np.zeros((nb_of_inequalities, KyberKEM.SHARED_SECRET_BYTES), dtype=np.uint8)
	for i in range(nb_of_inequalities):
		done = False
		nb_of_encapsulations = 0
		lowest_bias = 10000
		while not done:
			c, ss, m, r, e1, du, e2, dv, ss_pre = kem.encapsulate(public_key, return_internals=True)
			b2 = e2 + dv
			...
			if return_manipulation:
				...
				ciphertexts[i,:] = c
				manipulated_ciphertexts[i,:], _ = manipulate_ciphertext(kem.pke, c, ind)
				shared_secrets[i,:] = ss
				manipulated_shared_secrets[i,:] = KyberKEM.KDF(
					np.concatenate((ss_pre, KyberKEM.H(manipulated_ciphertexts[i,:]))))
			if bias[ind] < lowest_bias:
				lowest_bias = bias[ind]
				b[i] = b2[ind]
				for j in range(K):
					a[i, j*N:(j+1)*N] = poly_multiplier_to_matrix(-e1[j,:] - du[j,:], row_index=ind)
					a[i, (K+j)*N:(K+j+1)*N] = poly_multiplier_to_matrix(r[j,:], row_index=ind)
			nb_of_encapsulations += 1
			done = (bias_threshold is None) or (bias[ind] <= bias_threshold) or \
				((max_nb_of_encapsulations is not None) and \
				(nb_of_encapsulations >= max_nb_of_encapsulations))
	return [a, b, manipulated_indices, ciphertexts, manipulated_ciphertexts, \
		shared_secrets, manipulated_shared_secrets] if return_manipulation else [a, b]
...
def evaluate_inequalities_slow(
	...
	for i in range(nb_of_inequalities):
		...
		is_geq_zero[i] = not np.array_equal(ss, manipulated_shared_secrets[i,:])
	return is_geq_zero

def evaluate_inequalities_fast(a, b, solution):
		return (np.matmul(a, solution) + b) >= 0

def corrupt_inequalities(is_geq_zero, prob_success_is_missed):
...
def generate_equalities(kem, public_key, verbose=True):
	...
	a = np.hstack((np.zeros((K*N, K*N), dtype=np.int32), 2285*np.eye(K*N, dtype=np.int32)))
	b = np.zeros((K*N), dtype=np.int32)
	for i in range(K):
		b[i*N:(i+1)*N] = -kem.pke.INTT(t[i])
		for j in range(K):
			a[i*N:(i+1)*N,j*N:(j+1)*N] = poly_multiplier_to_matrix(kem.pke.INTT(A[i,j]))
	return a, b

def solve_inequalities(kem, a, b, is_geq_zero, max_nb_of_iterations=16, verbose=True, solution=None):
	if verbose:
	...
	p_inequality_is_correct = min(p_failure_is_reality / p_failure_is_observed, 1.0)
	...
	for z in range(max_nb_of_iterations):
		...
		psuccess = np.zeros((nb_of_values, nb_of_inequalities, nb_of_unknowns), dtype=float)
...
def test_inequalities():
	...
	for K in [2, 3, 4]:
		...
		[a, b, manipulated_indices, _, manipulated_ciphertexts, _, manipulated_shared_secrets] = \
			generate_inequalities(kem, public_key, nb_of_inequalities, return_manipulation=True)
		is_geq_zero1 = evaluate_inequalities_slow(
			kem, private_key, manipulated_indices, manipulated_ciphertexts, manipulated_shared_secrets)
