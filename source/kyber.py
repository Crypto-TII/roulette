...
class KyberPKE:
	...
	def keygen(self, return_internals=False):
		...
		return [public_key, private_key, s, e] if return_internals else [public_key, private_key]
	def encrypt(self, public_key, message, coins, return_internals=False, roulette_index = None):
		(t, rho) = np.split(public_key, [self.K*KyberPKE.N*3//2])
		t = np.reshape(KyberPKE.decode(t, 12, self.K*KyberPKE.N), (self.K, -1))
		...
		if roulette_index is not None:
			v2[roulette_index] = (v2[roulette_index] + 2**(self.DV - 2)) % (2**(self.DV))
	...
	def NTT(data):
		...
		for i in range(7):
			ind_a = np.tile(np.arange(L[i]), 2**i) + np.repeat(np.arange(256,step=2*L[i]), L[i])
			ind_b = ind_a + L[i]
			ind_z = np.repeat(np.arange(2**i, 2**(i+1)), L[i])
			(data[ind_a], data[ind_b]) = KyberPKE.CooleyTukeyButterflies(data[ind_a], data[ind_b], KyberPKE.ZETAS[ind_z])
	...
	def INTT(data):
		...
		for i in np.flip(np.arange(7)):
			...
			(data[ind_a], data[ind_b]) = KyberPKE.GentlemanSandeButterflies(data[ind_a], data[ind_b], KyberPKE.ZETASINV[ind_z])
			j += 2**i
		return KyberPKE.Montgomery_multiply(data, np.repeat(KyberPKE.ZETASINV[127], 256))
	@staticmethod
	def round_half_up(x):
...
class KyberKEM:
	...
	def keygen(self, return_internals=False):
		public_key, private_key, *internals = self.pke.keygen(return_internals=return_internals)
	...
	def encapsulate(self, public_key, return_internals=False):
		...
		c, *internals = self.pke.encrypt(public_key, m, r, return_internals=return_internals)
		k2 = np.concatenate((k, KyberKEM.H(c)))
		k2 = KyberKEM.KDF(k2)
		return [c, k2, m, *internals, k] if return_internals else [c, k2]
	def decapsulate(self, private_key, c, return_internals=False, roulette_index = None):
